...
namespace TE.FileWatcher.Configuration
{
    public abstract class PlaceholderBase
    {
        ...
        /// otherwise <c>null</c>.
        /// </returns>
        protected string? ReplaceFormatPlaceholders(string value, string watchPath, string fullPath)
        {
            if (string.IsNullOrWhiteSpace(value) || string.IsNullOrWhiteSpace(watchPath) || string.IsNullOrWhiteSpace(fullPath))
            {
                return null;
                        ...
                        try
                        {
                            ...
                            switch (type)
                            {
                                ...
                                case PLACEHOLDERCURRENTDATE:
                                    value = GetDateValue(match.Value, value, type, format, watchPath, fullPath);
                                    break;
                                case PLACEHOLDERENVVAR:
                                    value = GetEnvironmentVariableValue(match.Value, value, format, watchPath, fullPath);
                                    break;
                                case PLACEHOLDERURLENCODE:
                                    value = GetUrlEncodedValue(match.Value, value, format, watchPath, fullPath);
                                    break;
                            };
        ...
        /// </returns>
        private static string GetDateValue(string placeholder, string value, string type, string format, string watchPath, string fullPath)
        {
            ...
            {
                Logger.WriteLine(
                    $"The date value is null. Date type: {type}, changed: {fullPath}, value: {value}, watch path: {watchPath}.",
                    LogLevel.WARNING);
        ...
        /// </returns>
        private static string GetEnvironmentVariableValue(string placeholder, string value, string envName, string watchPath, string fullPath)
        {
            string? envValue = Environment.GetEnvironmentVariable(envName);
            if (envValue != null)
            {
                Logger.WriteLine($"envarvalue: {envValue}");
                value = value.Replace(placeholder, envValue, StringComparison.OrdinalIgnoreCase);
            }
            else
            {
                Logger.WriteLine(
                $"The environment variable value is null. Environment variable: {envName}, changed: {fullPath}, value: {value}, watch path: {watchPath}.",
                LogLevel.WARNING);
            }

            Logger.WriteLine($"returned: {value}");
            return value;
        }
        ...
        /// </param>
        /// <returns></returns>
        private static string GetUrlEncodedValue(string placeholder, string value, string url, string watchPath, string fullPath)
        {
            ...
            {
                Logger.WriteLine(
                    $"The URL value could not be encoded. URL: {url}, changed: {fullPath}, value: {value}, watch path: {watchPath}.",
                    LogLevel.WARNING);
            }
